...
export function getPositionCoordinates(
  ...
  const baseCoords = {
    ...
    [Position.LEFT_BACK]: { x: fieldMargin + fieldWidth * 0.22, y: height * 0.2 },
    [Position.CENTER_BACK]: { x: fieldMargin + fieldWidth * 0.22, y: height * 0.5 },
    [Position.RIGHT_BACK]: { x: fieldMargin + fieldWidth * 0.22, y: height * 0.8 },

    // Defensive Midfielders
    [Position.DEFENSIVE_MIDFIELDER]: { x: fieldMargin + fieldWidth * 0.4, y: height / 2 },

    // Midfielders - improved spacing
    [Position.LEFT_MIDFIELDER]: { x: fieldMargin + fieldWidth * 0.55, y: height * 0.35 },
    [Position.CENTER_MIDFIELDER]: { x: fieldMargin + fieldWidth * 0.4, y: height / 2 },
    [Position.RIGHT_MIDFIELDER]: { x: fieldMargin + fieldWidth * 0.55, y: height * 0.65 },
  ...
  const offsetCoords: PositionCoordinates = {} as PositionCoordinates;
  for (const [position, coords] of Object.entries(baseCoords)) {
    offsetCoords[position as Position] = {
